...
void action(struct json *j)	
{
	...
	{	
		printf(" -- Data published in Tangle %ld seconds ago --\n", j->relative_timestamp);
		...
		*****************************/
		...
		{	
			...
			printf("Temperature Sensor Example       -> High Temperature detected: Turning ON the Refrigeration Equipment\n\n");
		}
		...
		if (humidity > 40.0)			// If humidity of the soil is lower than 40%, Turn on Irrigation Equipment
		{
			...
			printf("Humidity Sensor Example       -> Low Humidity in the soil detected: Turning ON the Irrigation Equipment\n\n");
		}
		else if (temperature < 30.0)	// If humidity of the soil is higher than 40%, Turn off Irrigation Equipment
		...
		float x_accel = atof(j->sensor[5].value[0]);
		...
		{
			set_relay_GPIO(2, 1);  // Put in HIGH RELAY 2
		...
		{
			set_relay_GPIO(0, 1);  // Put in HIGH RELAY 2
...
bool get_data_tangle(char* js, long* c)
{
	++(*c);
	d_collect_msg(c);
	print_json(js);
